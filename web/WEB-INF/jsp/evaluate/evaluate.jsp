        ...
        <div class="col-10 rounded-div">
                    ...
                    <c:when test="${userRole eq 1 }">
                        <div class="col-auto">
                            ...
                            <a type="button" class="btn btn-danger btn-sm"
                               href="/supervision/addEvaluateList"
                            >添加</a>
                        </div>
                        <%--                            批量删除--%>
            ...
            <div>
                <table class="table  table-hover">
                    ...
                    <tbody>
                    <c:forEach var="findAllEvaluateList" items="${findAllEvaluateList}" varStatus="pages">
                        <tr>
                            <td>
                                <input class="form-check-input" type="checkbox" name="selectedUsers"
                                       value="${findAllEvaluateList.optionId}">
                            </td>
                            <td>${pages.index + 1}</td>
                            <td class="text-muted">
                                    ${findAllEvaluateList.scores.scoreName}
                            </td>
                            <td class="text-muted">
                                ...
                                <div class="text-truncate">
                                        ${findAllEvaluateList.optionName}
                                </div>
                            </td>
                                    ...
                                    <div class="dropdown-content" id="dropdown${pages.index}">
                                        ...
                                        <c:choose>
                                            <c:when test="${userRole eq 1 or userRole eq 2}">
                                                <a href="/supervision/updateEvaluateOptionId/${findAllEvaluateList.optionId}"
                                        ...
                                        <c:choose>
                                            <c:when test="${userRole eq 1 }">
                                                <a data-bs-toggle="modal" data-bs-target="#exampleModal"
                                                   onclick="deleteId(${findAllEvaluateList.optionId})"
...
<script>
    ...
    function deleteId(optionId) {
        // 在触发确认按钮后
        document.getElementById("delete-button").addEventListener("click", function () {
            //进行ajax
            $.ajax({
                url: '/supervision/deleteEvaluate/' + optionId,
                success: function (response) {
                    // 删除成功后的处理逻辑
                    console.log('删除成功');
                    //跳转业务
                    window.location.href = '/supervision/findAllEvaluate';
                },
