        ...
        <div class="col-10 rounded-div">
                    ...
                    <c:when test="${userRole eq 1 or userRole eq 2}">
                        <div class="col-auto">
                            ...
                            <a type="button" class="btn btn-danger btn-sm" style="display: none" id="buttonDivButton"
                               href="/supervision/UpSupervision"
                            >添加</a>
                        </div>
                        <%--                            批量删除--%>
            ...
            <div>
                <table class="table  table-hover">
                    ...
                    <tbody>
                    <c:forEach var="findAllSupervisionList" items="${findAllSupervisionList}" varStatus="pages">
                        <tr>
                            <td>
                                <input class="form-check-input" type="checkbox" name="selectedUsers"
                                       value="${findAllSupervisionList.supId}">
                            </td>
                            <td>${pages.index + 1}</td>
                            <td class="text-muted">
                                    ${findAllSupervisionList.supName}
                            </td>
                            <td class="text-muted">
                                ...
                                <div class="text-truncate">
                                        ${findAllSupervisionList.supTimeStart}
                                </div>

                            </td>
                            <td class="text-muted">
                                <div class="text-truncate">
                                        ${findAllSupervisionList.supTimeEnd}
                                </div>
                            </td>
                            <td class="text-muted">
                                    ${findAllSupervisionList.ask}
                            </td>
                            <td>
                                <div class="dropdown" onmouseleave="hideDropdown('dropdown${pages.index}')">
                                    ...
                                    <div class="dropdown-content" id="dropdown${pages.index}">
                                        ...
                                        <c:choose>
                                            <c:when test="${userRole eq 1 or userRole eq 2}">
                                                <a href="/supervision/updateSupervisionSuId/${findAllSupervisionList.supId}"
                                        ...
                                        <c:choose>
                                            <c:when test="${userRole eq 1 }">
                                                <a data-bs-toggle="modal" data-bs-target="#exampleModal"
                                                   onclick="deleteId(${findAllSupervisionList.supId})"
                                        ...
                                        <a href="/supervision/supervisionInspection/${findAllSupervisionList.supId}"
                                           id="Checks${pages.index}"
                                           class="content"
                                        >详细查看</a>


                                        <a href="/supervision/findFileSupervision?fUid=${findAllSupervisionList.supervisionFileSava.FUid}"
                                        >查看文件</a>
                                    </div>
                                </div>
                            </td>
...
<script>
    ...
    function deleteId(supId) {
        // 在触发确认按钮后
        document.getElementById("delete-button").addEventListener("click", function () {
            //进行ajax
            $.ajax({
                url: '/supervision/deleteSupervision/' + supId,
                success: function (response) {
                    // 删除成功后的处理逻辑
                    console.log('删除成功');
                    //跳转业务
                    window.location.href = '/supervision/findAllSupervision?target=Supervision';
                },
...
<script>
    ...
    document.getElementById("buttonDiv").addEventListener("click", function () {
        ...
        document.getElementById("confirmDeleteButton").addEventListener("click", function () {
            ...
            $.ajax({
                url: "/supervision/deletesupervisionbatch/",
                type: "POST",
                // 指定内容类型为JSON
