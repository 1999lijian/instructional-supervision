            ...
            <div class="container">
                ...
                <c:choose>
                    <c:when test="${userRole eq 2  }">
                        <button type="button" class="btn btn-primary" onclick="submitForm()">修改</button>
                    </c:when>
                </c:choose>
                <a type="button" class="btn btn-primary" href="/supervision/findAllCourse?evaluate=Listening">返回</a>
            </div>
...
<script>
    ...
    //把查询到的选项 遍历  设置为选择中
    <c:forEach var="courseListenDate" items="${courseListenDate}">
    var listencordsList = ${courseListenDate.listencordsList};
    // console.log(listencordsList);
    Object.keys(listencordsList).forEach(function (groupName) {
        var selectedValue = listencordsList[groupName];
        var radioButton = document.querySelector('input[name="' + groupName + '"][value="' + selectedValue + '"]');
        if (radioButton) {
    ...
    function submitForm() {
        ...
        if (allGroupsChecked) {
            ...
            $.ajax({
                type: 'POST',
                url: '/supervision/findCourseListenDateUP', // 后端处理页面的URL
                data: formData,
