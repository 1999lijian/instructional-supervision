...
<%--                        <div class="col-auto">--%>
            ...
            <div>
                <table class="table  table-hover">
                    ...
                    <tbody>
                    <c:forEach var="findAllClassesList" items="${findAllClassesList}" varStatus="pages">
                        <tr>
                            <td>
                                <input class="form-check-input" type="checkbox" name="selectedUsers"
                                       value="${findAllClassesList.CIId}">
                            </td>
                            <td>${pages.index + 1}</td>
                            <td class="text-muted">
                                    ${findAllClassesList.CIName}
                            </td>
                            <td>
                                <div class="dropdown" onmouseleave="hideDropdown('dropdown${pages.index}')">
                                    ...
                                    <div class="dropdown-content" id="dropdown${pages.index}">
                                        ...
                                        <c:choose>
                                            <c:when test="${userRole eq 1 }">
                                                <a href="/supervision/updateClasses/${findAllClassesList.CIId}"
                                        ...
                                        <c:choose>
                                            <c:when test="${userRole eq 1 }">
                                                <a data-bs-toggle="modal" data-bs-target="#exampleModal"
                                                   onclick="deleteId(${findAllClassesList.CIId})"
                                        ...
                                        <c:choose>
                                            <c:when test="${userRole eq 1 }">
                                                <a href="/supervision/addClasses/${findAllClassesList.CIId}"
                                                   id="SupervisionUp${pages.index}" class="content"
                                                >加入课程</a>
                                            </c:when>
                                        </c:choose>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="add">
                    <div class="form">
                        <label for="CIName">班级名称</label>
                        <input type="text" class="form-control" id="CIName" placeholder="请输入班级名称"
                               name="CIName">
                    </div>
                </form>
            </div>
...
<script>
    ...
    function deleteId(CIId) {
        // 在触发确认按钮后
        document.getElementById("delete-button").addEventListener("click", function () {
            //进行ajax
            $.ajax({
                url: '/supervision/deleteClass/' + CIId,
                success: function (response) {
                    // 删除成功后的处理逻辑
                    console.log('删除成功');
                    //跳转业务
                    window.location.href = '/supervision/findAllClasses';
                },
